...
def categorical(df):
    ...
    return categorical_var.columns.tolist()
...
def numerical(df):
    ...
    return numerical_var.columns.tolist()
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    #aggregated_value=pd.pivot_table(df,index=df['month'],values=agg_col,aggfunc=agg)
...
def convert(df,celsius):
    ...
    """
    converted_temp=(9/5)*df[celsius]+32
...
print('categorical:',categorical(weather))

print('numerical:',numerical(weather))
...
print('checking:',clear(weather,"Weather",'Clear'))
print('weather',clear(weather,'Weather',5))
...
print('wind_spees_35_vis_25:',wind_speed_35_vis_25)
...
print('weather mean:',mean_weather[0:5])
...
print('Temp conversion from celsius to Fahrenheit:',convert(weather,'Temp (C)'))
